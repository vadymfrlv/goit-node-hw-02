...
А також використаний пакет [gravatar](https://www.npmjs.com/package/gravatar) для того, щоб при
реєстрації нового користувача відразу згенерувати йому аватар по його `email`.
...
- Аватарка обробляється пакетом [jimp](https://www.npmjs.com/package/jimp) і ставиться розміри 250
  на 250 для неї.
- Переноситься аватарка користувача з папки `tmp` в папку `public/avatars` і дається їй унікальне ім'я
  для конкретного користувача (його айді з DB).
